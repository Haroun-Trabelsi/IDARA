...
import { getStatusDisplay, getStatusColor, getStatusIcon, getAvatarInitials, Member } from "utils/organizationUtils";
import { Account } from "../../@types/index";

interface CollaboratorPageProps {
  orgData: { name: string; organizationSize: number; id: string; region: string; teamSize: string };
  members: Member[];
...
export default function CollaboratorPage({
  ...
  const isGuest = !!account?.invitedBy && account?.invitedBy !== account?._id;
  ...
  const filteredMembers = members
    ...
    .filter((member) => {
      ...
      const showInMembers = isMemberTab && (isOwner || member.status === "accepted" || member.status === "AdministratorOrganization");
  ...
  return (
    <Box>
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 3 }}>
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          ...
          <Button
            ...
            disabled={
              !account?.canInvite ||
              (account?.invitedBy && account?.invitedBy !== account?._id) ||
              !account?._id ||
              (!account?.invitedBy &&
                members.some((m) => m.invitedBy === account?._id && ["pending", "accepted"].includes(m.status)))
            }
            sx={{
              bgcolor: "#4299e1",
      ...
      <Typography variant="body2" sx={{ color: "#a0aec0", pl: 1, mb: 2 }}>
        Only the organization admin can send invitations, with a limit of two members.
      </Typography>
      ...
      <Box sx={{ borderBottom: "1px solid #2d3748", mb: 3 }}>
        ...
        >
          <Tab label="Members" />
          <Tab label="Pending invites" disabled={isGuest} />
        </Tabs>
      </Box>
                  ...
                  <TableRow>
                    <TableCell colSpan={4} sx={{ textAlign: "center", py: 4, color: "#a0aec0" }}>
                      <Box sx={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 1 }}>
                        ...
                        <Typography>{membersTab === 1 ? "No pending invitations" : "No members found"}</Typography>
                      </Box>
                    </TableCell>
                  </TableRow>
                ) : (
                  filteredMembers.map((member) => (
                    <TableRow key={member._id} sx={{ borderBottom: "1px solid #2d3748" }}>
                        ...
                        <Box sx={{ display: "flex", alignItems: "center" }}>
                          <Avatar
                            sx={{
                              width: 32,
                              height: 32,
                              bgcolor: member.status === "AdministratorOrganization" && member.canInvite ? "#4299e1" : "#38b2ac",
                              fontSize: "14px",
                              mr: 2,
                            }}
                          >
                            ...
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                              ...
                              >
                                {member._id === account?._id
                                  ? ` ${member.name} ${member.surname || ""} (You)`
                      ...
                      <TableCell sx={{ borderBottom: "1px solid #2d3748", color: "#a0aec0", fontSize: "14px" }}>
                        {member.status === "AdministratorOrganization" && member.canInvite ? "Owner" : "Guest"}
                      </TableCell>
                      <TableCell sx={{ borderBottom: "1px solid #2d3748", color: "#a0aec0", fontSize: "14px" }}>
                        {member.invitedDate || "-"}
                      </TableCell>
