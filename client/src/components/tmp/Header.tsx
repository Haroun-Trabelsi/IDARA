...
import React, { useState, Fragment, MouseEventHandler } from "react"
...
import OnlineIndicator from 'components/OnlineIndicator'  // make sure you import it
    ...
    <AppBar position="static" elevation={0} sx={{ bgcolor: "#2d3748", borderBottom: "1px solid #4a5568" }}>
      <Toolbar sx={{ justifyContent: "space-between", minHeight: "48px !important", px: 2 }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 3 }}>
          <Button endIcon={<KeyboardArrowDown sx={{ fontSize: "16px" }} />} onClick={(e) => setProjectsAnchorEl(e.currentTarget)} sx={{ color: "#e2e8f0", textTransform: "none", fontSize: "14px", fontWeight: 400, "&:hover": { bgcolor: "rgba(255, 255, 255, 0.08)" } }}>
            Projects
          </Button>
          <Menu anchorEl={projectsAnchorEl} open={Boolean(projectsAnchorEl)} onClose={() => setProjectsAnchorEl(null)} PaperProps={{ sx: { bgcolor: "#2d3748", border: "1px solid #4a5568" } }}>
            <MenuItem sx={{ color: "#e2e8f0" }}>Project 1</MenuItem>
            <MenuItem sx={{ color: "#e2e8f0" }}>Project 2</MenuItem>
          </Menu>
